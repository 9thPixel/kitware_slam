...
#ifndef LEISHENTOLIDARNODE_H
#define LEISHENTOLIDARNODE_H
#include <ros/ros.h>
#include <pcl_ros/point_cloud.h>
#include <lidar_conversions/leishen_point_type.hpp>
...
/**
 * @class LeishenToLidarNode aims at converting pointclouds published by ROS
 * Leishen driver to the expected SLAM pointcloud format.
 *
 * The ROS Leishen driver can be found here :
 * https://github.com/leishen-lidar
 */
class LeishenToLidarNode
{
public:
    using PointV = leishen_pcl::PointXYZIRT;
    using CloudV = pcl::PointCloud<PointV>;  ///< Pointcloud published by Leishen driver
   ...
   */
    LeishenToLidarNode(ros::NodeHandle& nh, ros::NodeHandle& priv_nh);

    //----------------------------------------------------------------------------
    /*!
   * @brief New lidar frame callback, converting and publishing Leishen PointCloud as SLAM LidarPoint.
   * @param cloud New Lidar Frame, published by lslidar_XX_decoder_node.
   */
...
private:
    ...
    // NOTE: the Leishen ROS driver should already correctly modify the laser_id,
    ...
    // These parameters should be set to the same values as ROS Leishen driver's.
