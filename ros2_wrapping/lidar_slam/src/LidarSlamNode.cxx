...
void LidarSlamNode::SetSlamParameters()
{
  ...
  if (this->get_parameter("slam.ke.device_ids", deviceIds))
  {
    ...
    for (auto deviceId: deviceIds)
    {
      ...
      int keypointExtractorMode;
      if (this->get_parameter<int>(prefix + "extractor_mode", keypointExtractorMode))
      {
        LidarSlam::KeypointExtractorMode keypointExtractor = static_cast<LidarSlam::KeypointExtractorMode>(keypointExtractorMode);
        if (keypointExtractor != LidarSlam::KeypointExtractorMode::SPARSE &&
            keypointExtractor != LidarSlam::KeypointExtractorMode::DENSE)
        {
          RCLCPP_ERROR_STREAM(this->get_logger(), "Incorrect keypoint extractor mode value (" << keypointExtractorMode <<
                                                  ")" << "for device " << deviceId << ". Setting it to '0', corresponding to sparse extractor.");
          keypointExtractor = LidarSlam::KeypointExtractorMode::SPARSE;
        }
        if (keypointExtractor == LidarSlam::KeypointExtractorMode::SPARSE)
        {
  ...
  else
  {
    ...
    int keypointExtractorMode;
    if (this->get_parameter<int>("slam.ke.extractor_mode", keypointExtractorMode))
    {
      LidarSlam::KeypointExtractorMode keypointExtractor = static_cast<LidarSlam::KeypointExtractorMode>(keypointExtractorMode);
      if (keypointExtractor != LidarSlam::KeypointExtractorMode::SPARSE &&
          keypointExtractor != LidarSlam::KeypointExtractorMode::DENSE)
      {
        RCLCPP_ERROR_STREAM(this->get_logger(), "Incorrect keypoint extractor mode value (" << keypointExtractorMode <<
                                                "). Setting it to '0', corresponding to sparse extractor.");
        keypointExtractor = LidarSlam::KeypointExtractorMode::SPARSE;
      }
      if (keypointExtractor == LidarSlam::KeypointExtractorMode::SPARSE)
      {
